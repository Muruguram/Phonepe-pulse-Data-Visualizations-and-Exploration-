...
def transaction_locations__amount_all(Year):
        ...
        query1=(f"select states,sum(transactionamount) as Transaction, sum(Transactioncount) as count from aggregated_transaction where year={Year} group by states order by Transaction desc")
...
def transaction_locations_animation_year(Year,Quarter):
        ...
        response =requests.get(url)
        data1 = json.loads(response.content)
        query1=(f"select states,sum(transactionamount) as Transaction, sum(Transactioncount) as count from aggregated_transaction where year={Year} and quarter={Quarter} group by states order by Transaction desc")
        mycursor.execute(query1)
        table1=mycursor.fetchall()
        table=pd.DataFrame(table1,columns=["States","Transaction Amount","Transaction Count"])
...
def transaction_locations_count_all(Year):
        ...
        response =requests.get(url)
        data1 = json.loads(response.content)
        query1=(f"select states,sum(transactionamount) as Transaction, sum(Transactioncount) as count from aggregated_transaction where year={Year} group by states order by count desc")
        mycursor.execute(query1)
        table1=mycursor.fetchall()
        table=pd.DataFrame(table1,columns=["States","Transaction Amount","Transaction Count"])
...
def transaction_count_animation_year(Year,Quarter):
        ...
        response =requests.get(url)
        data1 = json.loads(response.content)
        query1=(f"select states,sum(transactionamount) as Transaction, sum(Transactioncount) as count from aggregated_transaction where year={Year} and quarter={Quarter} group by states order by count desc")
        mycursor.execute(query1)
        table1=mycursor.fetchall()
        table=pd.DataFrame(table1,columns=["States","Transaction Amount","Transaction Count"])
        transaction=px.choropleth(table, geojson= data1, locations= "States", 
...
def payment_transc_type_amount(Year):
        ...
        mycursor=mydb.cursor()
        query1=(f"select transactiontype,sum(transactionamount) as Amount from aggregated_transaction where years={Year} group by transaction_name order by Amount desc")
        mycursor.execute(query1)
        table1=mycursor.fetchall()
...
def payment_transc_type_all_amount(Year,Quarter):
        ...
        mycursor=mydb.cursor()
        query1=(f"select transactiontype,sum(transactionamount) as Amount from aggregated_transaction where year={Year} and quarter={Quarter} group by transactiontype order by Amount desc")
        mycursor.execute(query1)
        table1=mycursor.fetchall()
        table=pd.DataFrame(table1,columns=["Transaction Name","Transaction Amount"])
        Transc_fig= px.bar(table,x= "Transaction Name",y= "Transaction Amount",title= "TRANSACTION TYPE and TRANSACTION AMOUNT",
...
def registered_user(Year):
        ...
        response =requests.get(url)
        data1 = json.loads(response.content)
        query1=(f"select states,sum(registeredusers) as Users from map_user where years={Year} group by states order by Users desc")
        mycursor.execute(query1)
        table1=mycursor.fetchall()
...
def registered_user_all(Year,Quarter):
        ...
        response =requests.get(url)
        data1 = json.loads(response.content)
        query1=(f"select states,sum(registeredusers) as Users from map_user where year={Year} and quarter={Quarter} group by states order by Users desc")
        mycursor.execute(query1)
        table1=mycursor.fetchall()
        table=pd.DataFrame(table1,columns=["States","Registered Users"])
        transaction=px.choropleth(table, geojson= data1, locations= "States", 
...
def payment_transc_type_count(Year):
        ...
        mycursor=mydb.cursor()
        query1=(f"select transactiontype,sum(transactioncount) as Count from aggregated_transaction where years={Year} group by transaction_name order by Count desc")
        mycursor.execute(query1)
        table1=mycursor.fetchall()
...
def payment_transc_type_count_all(Year,Quarter):
        ...
        mycursor=mydb.cursor()
        query1=(f"select transactiontype,sum(transactioncount) as Amount from aggregated_transaction where year={Year} and quarter={Quarter} group by transactiontype order by Amount desc")
        mycursor.execute(query1)
        table1=mycursor.fetchall()
        table=pd.DataFrame(table1,columns=["Transaction Name","Transaction Count"])
        Transc_fig= px.bar(table,x= "Transaction Name",y= "Transaction Count",title= "TRANSACTION TYPE and TRANSACTION COUNT",
...
st.title(":white[PhonePe Pulse Data Visulaization And Exploration]")


SELECT = option_menu(
        menu_title = None,
        options = ["About","Top Insights","Top Charts"],
        ...
        styles={"container": {"padding": "0!important", "background-color": "black","size":"cover", "width": "100%"},
                ...
                "nav-link-selected": {"background-color": "#e803fc"}})
if SELECT=="About":
...
if SELECT=="Top Charts":
        st.subheader("Choose the options below to know about facts of the data")
        ...
        if options=="Top brands of mobile used":
                ...
                if Year!="Select Year to Know about Brands":
                        query1=(f"select brands,sum(transactioncount) as Count from aggregated_user where year={Year} group by brands order by Count desc")
                        mycursor.execute(query1)
                ...
                if Year!="Select Year for lowest transaction":
                        query2=(f"select districts,sum(transactionamount) as Amount from map_transaction where year={Year} group by districts order by Amount asc")
                ...
                if Year!="Select Year for highest transaction":
                        query3=(f"select districts,sum(transactionamount) as Amount from map_transaction where year={Year} group by districts order by Amount desc")
                ...
                query4=("select year,sum(registeredusers) as users from map_user group by year order by year")
                ...
                if Year!="Select Year for highest PhonePe User":
                        query5=(f"select states,sum(registeredusers) as users from map_user where year={Year} group by states order by users desc")
                ...
                Year=st.selectbox("Year",("Select Year for highest PhonePe User",2018,2019,2020,2021,2022))
                if Year!="Select Year for highest PhonePe User":
                        query5=(f"select states,sum(registeredusers) as users from map_user where year={Year} group by states order by users asc")
                        mycursor.execute(query5)
                        table=mycursor.fetchall()
                        table2=pd.DataFrame(table,columns=["State","Registered Users"])
                        tab=table2.head(10)
                ...
                Year=st.selectbox("Year",("Select Year for highest transaction",2018,2019,2020,2021,2022))
                if Year!="Select Year for highest transaction":
                        query3=(f"select districts,sum(registeredusers) as users from map_user where year={Year} group by districts order by users desc")
                        mycursor.execute(query3)
                        table=mycursor.fetchall()
                ...
                Year=st.selectbox("Year",("Select Year for highest transaction",2018,2019,2020,2021,2022))
                if Year!="Select Year for highest transaction":
                        query3=(f"select districts,sum(registeredusers) as users from map_user where year={Year} group by districts order by users asc")
                        mycursor.execute(query3)
                        table=mycursor.fetchall()
                        table2=pd.DataFrame(table,columns=["District","Registered Users"])
                        tab=table2.head(10)
